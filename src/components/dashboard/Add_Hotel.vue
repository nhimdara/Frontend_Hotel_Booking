<template>
  <div class="min-h-screen bg-slate-50 p-4 font-sans text-slate-800 sm:p-6">
    <div class="w-full">
      ...
      <h2 class="mt-2 text-xl font-bold tracking-tight text-emerald-900 sm:text-2xl">
        Create Hotel
      </h2>
      ...
      <form class="mt-6 space-y-5" @submit.prevent="handleSubmit">
        <section class="rounded-2xl border border-slate-200/70 bg-white p-4 shadow-sm sm:p-6">
          ...
          <div class="mt-4">
            <span class="mb-1.5 block text-sm font-medium text-slate-700">Image file</span>
            <label class="flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50/60 px-4 py-5 text-center transition hover:border-emerald-300 hover:bg-emerald-50/50">
              <img
                v-if="imagePreview"
                :src="imagePreview"
          ...
          <label class="mt-4 block">
            ...
            <textarea v-model="form.description" rows="5" class="field resize-none" placeholder="Describe the property, location, and guest experience."></textarea>
          </label>
          <label class="mt-4 block">
        ...
        <div class="flex justify-end gap-3 pb-4">
          <button type="button" class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50" @click="router.push({ name: 'rooms' })">
            Cancel
          </button>
          <button type="submit" :disabled="saving" class="rounded-xl bg-emerald-800 px-5 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-emerald-900 disabled:cursor-not-allowed disabled:opacity-60">
...
function hotelPayload() {
  ...
  if (!imageFile.value) return payload;
...
async function handleSubmit() {
  ...
  try {
    await hotelApi.create(hotelPayload());
